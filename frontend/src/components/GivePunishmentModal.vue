...
function next() {
  ...
  if (step.value < 3) step.value = (step.value + 1) as any;
}
...
async function submit() {
  ...
  try {
    ...
    submitError.value = e instanceof Error ? e.message : "Kunde inte ge straff";
  } finally {
    submitting.value = false;
  }
}
    ...
    <div class="modal-box">
      <form class="flex flex-col gap-6" @submit.prevent="submit">
        ...
        <ul class="steps w-full">
          ...
          <li class="step" :class="{ 'step-primary': step >= 2 }">Anledning</li>
          <li class="step" :class="{ 'step-primary': step >= 3 }">Antal</li>
        </ul>

        <!-- STEP 1: choose user (grid) -->
        <div v-if="step === 1" class="flex flex-col gap-3">
          <div class="text-2xl text-center">Till vem?</div>
        ...
        <div v-else-if="step === 2" class="flex flex-col gap-3">
          <div class="text-2xl text-center">Anledning?</div>

          <textarea
            v-model="reason"
            class="textarea textarea-lg h-36"
        ...
        <!-- Errors -->
        <div v-if="submitError" class="text-sm text-error">
          {{ submitError }}
        </div>
